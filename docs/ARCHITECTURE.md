...
```
概览(默认首页): 今日数据 + 采集应用 + 热力图 + 快捷开关 + 类别排行 + 应用排行
语音输入 tab:  🎤 录音热键(hold/press) / 🔊 ASR 模型服务(chip+表单+测试) / 🎙 音频设备
行为采集 tab:  📋 采集应用(白名单卡片+类别chip) / ⚙️ 采集规则
图像识别 tab:  📷 VLM 屏幕理解(定时/按需) / 🔮 VLM 模型服务 / 🖼 画面采集范围 / 📊 采集参数
文字润色 tab:  ✨ 自动润色 / 🤖 LLM 模型服务 / 📝 润色提示词
个人提示词 tab: ⌨️ 快捷注入(前缀键) / 📋 个人提示词列表
快捷工具 tab:  📸 桌面截图 / 📁 数据管理
```

- 白名单:卡片式(图标+名+类别chip),添加走"选窗口遮罩交互"或"扫描应用"
- 类别:点 chip 选 5 选项改;空白开始,添加时分类(默认 other)
- 白名单增删改**即时生效**,表单配置走底部**统一保存条**(只有 [保存],无重置)
- 概览页快捷开关切换**自动保存**+ toast

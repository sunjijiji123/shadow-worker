...
static bool ggml_thread_apply_priority(int32_t prio) {
    ...
    if (prio != GGML_SCHED_PRIO_LOW) {
        ...
        // MinGW (w64devkit) headers may only ship the process-level throttling
        // API; the thread-level counterparts were added in newer Windows 11 SDKs.
        // Provide fallback definitions so the build succeeds on MinGW while the
        // call simply fails at runtime on older SDKs/Windows.

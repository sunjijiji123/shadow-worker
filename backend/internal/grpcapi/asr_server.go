...
// NewAsrServer 创建 AsrServer 实例。
func NewAsrServer(db *storage.DB, engine asr.Engine, logger *slog.Logger) *AsrServer {
	if logger == nil {
		logger = slog.Default()
	}
	return &AsrServer{db: db, engine: engine, logger: logger}
}
...
func (s *AsrServer) StreamRecognize(stream AsrService_StreamRecognizeServer) error {
	...
	if _, err := s.db.InsertEvent(storage.Event{
		...
		AppName: appName,
		Content: text,
		Meta:    fmt.Sprintf("engine=%s;duration_ms=%d", s.engine.Name(), elapsed.Milliseconds()),
	}); err != nil {

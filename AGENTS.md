...
54. **【采集】`GetForegroundWindow` 返回 NULL（显示器断电/锁屏/Session 切换）期间会被当成长时间"在用某应用"→ 跨断电期的巨怪段**：断电/锁屏时 `GetForegroundWindow` 持续返回 0（早期误判是"睡眠"，但日志显示 tick 每 300ms 连续打印、进程未冻结，是 Connected Standby/显示器断电而非睡眠）。`ForegroundApp()` 返回 `没有前台窗口` 错误。**历史 bug**：movement.go 的 loop 在 `ForegroundApp()` 失败时直接 `continue`，**绕过了离开检测**（`inputIdleMs >= awayThresholdMs` 那段逻辑在失败分支之后，到不了）→ `curSegID`/`away` 不更新。前台恢复后 `updateSegment` 因"同一应用"段合并语义（坑 31：`needNew := curSegID==0 || curApp.Path != app.Path`）发现还是同一 app，就**继续延长旧段的 end_ts**，把整个断电期粘进去——产生一条覆盖数小时（实测 9h44m，00:38 锁屏→10:22 恢复）的 engaged 段，时间轴上表现为"一直在用 ZCode 没动过"。**修复**：`ForegroundApp()` 失败时视为离开，调 `enterAway("no_foreground", now)` 立即断段（断电无法用 `inputIdleMs` 反推最后输入时刻，段在"发现无前台的当前时刻"收尾足够干净）；断段收尾逻辑已抽成公共 `enterAway(reason, end)` helper，键鼠离开路径（`input_timeout`）与新路径共用。**判别信号**：日志里连续多条 `level=DEBUG msg=获取前台应用失败 err=没有前台窗口`（每 300ms 一条）跨越问题时段，但问题段仍是 engaged（断电后从未离开）。断电行为无法用 inputIdleMs 反推最后输入时刻，故 `enterAway` 的 `end` 参数对 `no_foreground` 传 `time.Now()`，对 `input_timeout` 传 `now - inputIdleMs`。

55. **【MCP】`get_worklog` 日志量大时撑爆 LLM 上下文（三重 token 预算控制）**：MCP 工具返回的 JSON 直接进 agent 上下文，一个繁忙工作日（实测 6-28：217 段 + 445 事件）全量返回 **58.5KB ≈ 15,000 tokens**，对小窗口模型直接超限、对大窗口模型也占掉近半上下文。**根因有两层**：① segments 全量返回；② events 全量返回且 `vlm_summary` 是 token 大户（实测 6-28 有 422 条 vlm_summary，每条含 VLM 屏幕识别摘要文本，合计 ~16KB，是 voice 事件的 9 倍体量）。**三重控制（已实现）**：① `get_worklog` 的 segments 加 `limit`/`offset` 分页（默认 50，`total_segments`/`has_more` 供 agent 判断翻页，`TotalActiveMinutes`/`TotalSegments` 统计基于过滤后全集不受分页影响）；② `get_worklog` 的 events 加 `event_types` 参数，**默认只返回 voice**（语音转写对日报有价值且量小），`vlm_summary` 高频碎需显式 `event_types:["vlm_summary"]` 开启，业务默认值由 server 层（`handleGetWorklog`）决定、storage 层只做纯过滤；③ 新增 `get_worklog_summary` 工具，三时段（上午/下午/晚上，基于 `StartTS.Local().Hour()` 划分，不受 `parseDate` 的 UTC 影响——坑 44）聚合每段的类别分钟数 + top3 应用，**压缩到 ~1KB（254 tokens，压缩比 58 倍）**，工具描述引导 agent "先 summary 看全局再 worklog 下钻"。**效果**：默认 `get_worklog` 58.5KB→7.1KB（↓87%），`get_worklog_summary` 1.0KB（↓98%）。**设计原则**：MCP 工具的返回要有 token 预算意识——概览/聚合工具优先（summary），详情工具默认限制 + 分页（limit/offset），高频低价值数据（vlm_summary）默认不返回、按需开启。注意 `WorklogResult.TotalEvents` 统计的是**过滤后**事件数（与 segments 的 total 语义一致）。泛型 helper `sortStats[T minuteKeyer]`（Go 1.26+）用于桶内类别/应用降序排序，避免额外 import `sort`。
